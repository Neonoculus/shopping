...
public class GoodsDaoImpl implements GoodsDao {
    ...
    public List<Goods> getGoodsByCIdAndTidsPage(int cid, List<Integer> tid, int start, int rows) {
        try {
            ...
            sql.append(" group by goods.g_id limit ?,?;");
            Object[] params = tid.toArray(new Object[tid.size() + 3]);
            params[0] = cid;
            for (int i = 0; i < tid.size(); i++) {
                params[i+1] = tid.get(i);
            }
            params[tid.size()+1] = start;
            params[tid.size()+2] = rows;
            List<Goods> goods = runner.query(sql.toString(), new BeanListHandler<>(Goods.class), params);
            return goods;
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public List<Goods> getGoodByAllQueryPage(String word, int start, int rows) {
        try {
            List<Goods> goods = runner.query("SELECT goods.g_id,goods.c_id,goods.m_id, goods.`name`,goods.photo,goods.price,goods.count,goods.`describe`,goods.`status`\n" +
                    ...
                    "\t\t\ttag.`name` like ? OR\n" +
                    "\t\t\tmerchant.`name` like ? \n" +
                    "GROUP BY goods.g_id limit ?,?;",new BeanListHandler<Goods>(Goods.class), "%" + word + "%","%" + word + "%","%" + word + "%","%" + word + "%",start,rows);
            return goods;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
...
//    rows代表每次取出多少条
    ...
    @Override
    public List<Goods> getGoodsPriceByCId(int cid,int minPrice,int maxPrice) {
        try {
            List<Goods> goods = runner.query("SELECT *  \n" +
                    "FROM goods  \n" +
                    "JOIN category ON category.c_id = goods.c_id  \n" +
                    "WHERE goods.c_id = ? AND goods.price BETWEEN ? AND ?;",new BeanListHandler<>(Goods.class),cid,minPrice,maxPrice);
            return goods;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public List<Goods> getGoodsPriceByCIdPage(int cid,int minPrice,int maxPrice,int start,int rows) {
        try {
            List<Goods> goods = runner.query("SELECT *  \n" +
                    "FROM goods  \n" +
                    "JOIN category ON category.c_id = goods.c_id  \n" +
                    "WHERE goods.c_id = ? AND goods.price BETWEEN ? AND ? limit ?,? ;",new BeanListHandler<>(Goods.class),cid,minPrice,maxPrice,start,rows);
            return goods;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
